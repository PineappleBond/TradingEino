...
import (
	...
	requests "github.com/PineappleBond/TradingEino/backend/pkg/okex/requests/rest/market"
	responses "github.com/PineappleBond/TradingEino/backend/pkg/okex/responses/market"
)
...
func (c *Market) GetOrderBookFull(req requests.GetOrderBook) (response responses.OrderBookFull, err error) {
	p := "/api/v5/market/books-full"
	m := okex.S2M(req)
	res, err := c.client.Do(http.MethodGet, p, false, m)
	if err != nil {
		return
