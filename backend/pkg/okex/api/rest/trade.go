...
import (
	...
	requests "github.com/PineappleBond/TradingEino/backend/pkg/okex/requests/rest/trade"
	responses "github.com/PineappleBond/TradingEino/backend/pkg/okex/responses/trade"
)
...
func (c *Trade) CandleOrder(req []requests.CancelOrder) (response responses.CancelOrder, err error) {
	...
	if len(req) > 1 {
		m := okex.S2MS(tmp)
		res, err = c.client.DoSliceBody(http.MethodPost, p, true, m)
	} else {
		m := okex.S2M(tmp)
		res, err = c.client.Do(http.MethodPost, p, true, m)
	}
