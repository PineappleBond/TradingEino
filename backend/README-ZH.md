...
```
定时任务触发 → OKXWatcher 协调 → 多 Agent 分析 → OKXWatcher 汇总 → ExecutorAgent 执行 → 完美闭环
```
...
- ✅ 定时任务调度系统（Cron）
- ✅ OKXWatcher 多 Agent 协调
- ✅ 技术分析 Agent（20+ 指标）
- ✅ 情绪分析 Agent（资金费率）
- ✅ 持仓管理 Agent
- ✅ Executor 执行 Agent（Level 1）
- ✅ 止损/止盈订单支持
- ✅ Web 管理界面
...
| Executor 从 Level 1 开始         | 仅执行明确指令，逐步建立信任                   | ✅ 已批准 |
...
| `okx-place-order-with-sl-tp` | 下单同时设置止损/止盈 | 5 req/s  |
| `okx-attach-sl-tp`     | 为已有订单附加止损/止盈 | 5 req/s  |
